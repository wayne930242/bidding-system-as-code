...
private fun BiddingTree.secondNegativeAfterMajor() {
    ...
    "3D" - "4+D支持，有控制"
    "3H" - "3+H支持（如為S排組再叫後）"
    "4H" - "快速到達：3+H支持，無一輪二輪控制（如為H排組）"
    "4S" - "快速到達：3+S支持，無一輪二輪控制（如為S排組）"
}
...
private fun BiddingTree.askingBidAfterSolidSuit() {
    remark(responder, "叫出有A的花色排組（多個A時叫最經濟的）")
...
fun BiddingTree.twoDiamondMulti() {
    explanation = "三色排組：4-4-4-1或4-4-5-0，10-13點，短排組為低花"
...
fun BiddingTree.weakTwoHeart() {
    explanation = "弱二：6張H排組，5-11點，最多3個控制"
...
fun BiddingTree.weakTwoSpade() {
    explanation = "弱二：6張S排組，5-11點，最多3個控制"
...
fun BiddingTree.twoNoTrump() {
    explanation = "21-22點均衡型（20點有5張排組可增值）"
...
fun BiddingTree.threeLevel() {
    explanation = "阻擊開叫：7張排組（低花可6張），5-7贏墩，最多1.5防守贏張"
...
fun BiddingTree.namyatsHeart() {
    explanation = "逆司臺曼：好的H排組（缺K或Q但不缺A），8-8.5贏墩"
...
fun BiddingTree.namyatsSpade() {
    explanation = "逆司臺曼：好的S排組（缺K或Q但不缺A），8-8.5贏墩"
